...
export async function spendFromEscrow(
    escrow: CardEscrowContract,
    caller: AccountWallet,
    amount: bigint,
    epoch: Fr,
...
export async function prepareChangeSpendLimit(
    escrow: CardEscrowContract,
    caller: AccountWallet,
    newLimit: bigint,
    opts: { send?: SendMethodOptions, wait?: WaitOpts } = {}
    ...
    const receipt = await escrow
        .methods
        .change_spend_limit(newLimit)
        .send(opts.send)
        .wait(opts.wait);
    return receipt.txHash;
}

export async function finalizeChangeSpendLimit(
    escrow: CardEscrowContract,
    caller: AccountWallet,
    opts: { send?: SendMethodOptions, wait?: WaitOpts } = {}
): Promise<TxHash> {
...
export async function cancelTimelocks(
    escrow: CardEscrowContract,
    caller: AccountWallet,
    cancelWithdraw: boolean,
    cancelSpendLimitChange: boolean,
    opts: { send?: SendMethodOptions, wait?: WaitOpts } = {}
): Promise<TxHash> {
    escrow = escrow.withWallet(caller);
    const receipt = await escrow
        .methods
        .cancel_timelocks(cancelWithdraw, cancelSpendLimitChange)
        .send(opts.send)
        .wait(opts.wait);
    return receipt.txHash;
}
